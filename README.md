...
- Quitar la referencia explícita a `System.Xml.Linq` en `CountrySoapHost.csproj` (no es necesaria en .NET 9 y genera una advertencia NU1701).
- Durante pruebas locales, puedes comentar `app.UseHttpsRedirection()` en `Program.cs` para evitar redirecciones automáticas a HTTPS si prefieres probar sólo HTTP.

Si quieres, puedo aplicar automáticamente cualquiera de esos cambios (quitar la referencia o comentar `UseHttpsRedirection`) y ejecutar el proyecto para verificarlo.